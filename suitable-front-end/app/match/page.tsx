...
export default function Match() {
  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col justify-between gap-8 row-start-2 items-center sm:items-start">
        <h1 className='text-red-600' style={{
          background: 'linear-gradient(to right, #FF512F, #DD2476)', // Gradient colors
          ...
          >
            <Card isFooterBlurred radius="lg" className="border-none w-full">
              <Image
                ...
                width={300}
              />
              <CardFooter className="before:bg-white/10 border-white/20 h-[50px] border-1 overflow-hidden py-1 absolute before:rounded-xl rounded-large bottom-1 w-[calc(100%_-_8px)] shadow-small ml-1 z-10">
                <h1 className="text-tiny text-white/80 text-xl">{matches[currentIndex].name}, 21</h1>
              </CardFooter>
            </Card>
          </div>
        )}
